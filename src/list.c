...
void list_destroy(struct list_t *list){
	if(list != NULL){
		...
		while(currentNode != NULL){
			...
			nextNode =(struct node_t *) currentNode->next;
			//delete node
			...
			// Porque fazes cast?
			currentNode = (struct node_t *) nextNode;
		}	
		free(list);
	}//list aponta para null
}
...
int list_add(struct list_t *list, struct entry_t *entry){
 // Verifica parametros
  ...
  if (list->head == NULL) {
    list->head = (struct node_t *)node_create(entry);
    if (list->head == NULL) {
      return ERROR;
    }
    list->size++;
...
int list_remove(struct list_t *list, char* key){
	...
	//check if is head
	if(strcmp(current->value->key , key) == EQUALS){
		list->head =(struct node_t *) current->next;
		node_destroy(current);
		//update listSize
		list->size--;
		return OK;
	}else{ //is not head, so is somewhere
		//while next is not null, run list
		while(current->next != NULL){
			//if next is entry key, delete this node
			if(strcmp(current->next->value->key, key) == EQUALS){
				struct node_t *toDelete =(struct node_t *) current->next;
				current->next =(struct node_t *) toDelete->next;
				...
				//else go to next
				current =(struct node_t *) current->next;
			}
		}
		//ran all list and did not find
		return ERROR;
	}
}
...
struct entry_t *list_get(struct list_t *list, char *key){
	//fist find list_t element with that entry
	struct node_t *current =(struct node_t *) list->head;
	//run list
	while(current != NULL){
		//if equals
		if(strcmp(current->value->key,key) == EQUALS){
...
void node_destroy(struct node_t *toDelete){
		//TODO
	if(toDelete != NULL){
		entry_destroy(toDelete->value); //apaga o entry
		toDelete->next = NULL; // não sei se é preciso
		free(toDelete);
	}
}
...
int sortList(struct list_t *list, struct node_t *newNode) {
	...
	while (currentNode->next != NULL) {
		...
		if (compareResult < 0) {
			...
			node_destroy(aux);
			return NOTHING_CHANGED;
		}else { // nao é menor nem igual, eh maior
